// server.js (VERSÃO DEBUG EXTREMO: LOG DE ARRAY + FIX COORDENADAS)
console.log("--- SERVIDOR DEBUG: VERIFICANDO DADOS E COORDENADAS ---");
...
const getValFromRow = (row, headerMap, possibleKeys) => {
    for (const key of possibleKeys) {
        ...
        if (idx !== undefined && row[idx] !== undefined) return parseSisfoCurrency(row[idx]);
    }
    return 0;
};
...
app.post('/api/update-base', async (req, res) => res.status(200).json({ message: 'Ok' }));
app.post('/api/update-event-status', async (req, res) => res.status(200).json({ message: 'Ok' }));
...
app.post('/api/cloud-sync', async (req, res) => {
  ...
  try {
    ...
    const processSheet = async (data, sheetName, headerRef) => {
        ...
        // 2. Prepara Dados (BLINDAGEM CONTRA NULOS)
        const rows = data.map(c => {
             if (isWaiterSheet) {
                 ...
                 // Garante string vazia se undefined, e faz trim
                 const ts = String(c.timestamp || '').trim();
                 const proto = String(c.protocol || '').trim();
                 const type = String(c.type || 'waiter').trim();
        ...
        // 3. Verifica Duplicatas e Calcula Linha
        ...
        if (nextRowIndex < 2) nextRowIndex = 2; // Garante que não sobrescreve Header
        ...
        if (toAdd.length > 0) {
            ...
            console.log(`>>> [GRAVANDO] Aba: ${sheetName}`);
            console.log(`>>> [COORDENADA] ${targetRange}`);
            console.log(`>>> [DADOS DUMP]`, JSON.stringify(toAdd[0])); // Verifica o conteúdo do primeiro item
        ...
        if (toUpdate.length > 0) {
            console.log(`>>> [ATUALIZANDO] ${toUpdate.length} itens.`);
