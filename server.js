// server.js (VERSÃO FINAL COMPLETA - SEM ABREVIAÇÕES)
// Correção aplicada: Update forçado na Coluna A e blindagem de dados nulos.

console.log("--- INICIANDO SERVIDOR COMPLETO (FULL 2.0) ---");
...
// --- ROTA 4: SYNC PARA A NUVEM (CORRIGIDA: UPDATE NA COLUNA A E BLINDAGEM DE DADOS) ---
app.post('/api/cloud-sync', async (req, res) => {
  ...
  try {
    ...
    const processSheet = async (data, sheetName, headerRef) => {
        ...
        // 2. Prepara os dados (Mapeamento com Blindagem de Nulos)
        const rows = data.map(c => {
             if (isWaiterSheet) {
                 ...
                 // BLINDAGEM: Garante que strings vazias sejam enviadas como '' e não undefined
                 const timestamp = c.timestamp || '';
                 const protocol = c.protocol || '';
                 const type = c.type || 'waiter';
                 const cpf = c.cpf || '';
                 const waiterName = c.waiterName || '';
                 const numeroMaquina = c.numeroMaquina || '';
                 const operatorName = c.operatorName || '';
                 ...
                 if (sheetName.includes('GarçomZIG')) {
                     ...
                     return [
                        timestamp, protocol, type, cpf, waiterName, numeroMaquina,
        ...
        if (toAdd.length > 0) {
            ...
            // Usa UPDATE direto na coordenada calculada
            await googleSheets.spreadsheets.values.update({ 
                spreadsheetId: spreadsheetId_cloud_sync, 
                range: targetRange,
                valueInputOption: 'USER_ENTERED', 
