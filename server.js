// server.js (VERSÃO COMPLETA - SEM RESUMOS - COM CORREÇÃO DE CABEÇALHO E EXTRAS)
console.log("--- EXECUTANDO VERSÃO FINAL (SISFO + AUTO-FIX + EXTRAS + CONCILIAÇÃO FULL) ---"); 
...
// --- ROTA DE SYNC PARA A NUVEM (COM CORREÇÃO DE CABEÇALHO) ---
        ...
        const rows = data.map(c => {
             if (sheetName.includes('GarçomZIG')) {
                 ...
                 return [
                    ...
                    c.valorTotalProdutos ?? 0, // Campo ZIG
...
// --- ROTA DE EXPORTAÇÃO (COM LEITURA DE EXTRAS PARA RECUPERAR OPERADOR) ---
    ...
    const fetchWithExtras = async (sheetName) => {
        try {
            ...
            return rows.map(row => {
                ...
                // Se o cabeçalho na nuvem estiver curto (ex: 14 cols) e o dado tiver 15,
                // a coluna 15 (Operador) será salva em EXTRA_14. O Frontend vai procurar aqui.
            ...
            if (header && rows.length > 0) {
                allClosings.push(...rows.map(row => {
                    ...
                    return {
                        ...
                        diferencaPagarReceber: Math.abs(parseSisfoCurrency(rowObj['ACERTO'])),
                        diferencaLabel: parseSisfoCurrency(rowObj['ACERTO']) >= 0 ? 'Receber do Garçom' : 'Pagar ao Garçom',
            ...
            if (header && rows.length > 0) {
                allClosings.push(...rows.map(row => {
                    ...
                    const base = {
                        ...
                        valorAcerto: parseSisfoCurrency(rowObj['VALOR ACERTO']), diferenca: parseSisfoCurrency(rowObj['DIFERENÇA']),
